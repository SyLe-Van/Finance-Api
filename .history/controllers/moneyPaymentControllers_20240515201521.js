...
module.exports = {
  ...
  updatePayList: async (req, res) => {
    try {
      ...
      const updates = req.body; // Assuming updates is an array of pay list items to update
      ...
      for (const update of updates) {
        const paylistIndex = group.pay_list.findIndex(
          (pay_list) => pay_list._id.toString() === update.paylistId
        );

        if (paylistIndex === -1) {
          console.error(`Member not found for paylistId: ${update.paylistId}`);
          continue; // Move to the next update if member not found
        }

        // Update paylist with new values
        group.pay_list[paylistIndex].member_id = update.member_id;
        group.pay_list[paylistIndex].member_name = update.member_name;
        group.pay_list[paylistIndex].value = update.value;
        group.pay_list[paylistIndex].note = update.note;
      }
